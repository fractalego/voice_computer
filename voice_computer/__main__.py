...
async def run_websocket_server(host: str, port: int, config_path: Optional[str] = None, verbose: bool = False):
    ...
    # Create a shared voice listener (without websocket callback)
    shared_voice_listener = ServerVoiceListener(config)
    
    # Create a temporary handler to initialize shared resources
    temp_handler = ConversationHandler(config)
    
    # Setup MCP tools and initialize models
    ...
    # Get the shared MCP tools and models
    shared_mcp_tools = temp_handler.mcp_tools
    shared_model_factory = temp_handler.model_factory
    shared_entailer = temp_handler.entailer
    shared_extractor = temp_handler.extractor
    ...
    async def handle_client(websocket):
        ...
        # Use pre-loaded shared resources instead of creating new ones
        handler.mcp_tools = shared_mcp_tools
        handler.model_factory = shared_model_factory
        handler.entailer = shared_entailer
        handler.extractor = shared_extractor
        handler.tool_handler = shared_tool_handler
        
        # The models are already initialized, no need to reload them
        logger.debug(f"Client {client_id} using shared pre-loaded resources")
